...
class OptimizationValidator {
  ...
  async validateSecurity() {
    ...
    if (manifestExists) {
      ...
      // Properly validate host permissions as URLs or match against allowed hostnames using URL parsing
      const allowedHostnames = new Set(['github.com', 'console.cloud.google.com']);
      const hasSpecificDomains = manifest.host_permissions?.some(p => {
        try {
          // Accept patterns like "https://*.github.com/*" by normalizing and extracting host
          // If permission is a pattern, try to coerce to a URL for hostname extraction
          // Prepend scheme if missing
          const candidate = p.startsWith('http') ? p : `https://${p.replace(/^\*:\/\//, '')}`;
          // Replace wildcard subdomains with placeholder to allow URL parsing
          const normalized = candidate.replace(/^https:\/\/*\./, 'https://sub.');
          if (!validator.isURL(normalized, { require_protocol: true })) return false;
          const host = new URL(normalized).hostname;
          // Handle wildcard subdomains: treat sub.github.com as github.com allowed
          return host === 'github.com' || host.endsWith('.github.com') ||
                 host === 'console.cloud.google.com' || host.endsWith('.console.cloud.google.com');
        } catch {
          return false;
        }
      }) || false;
  ...
  async validateFeatures() {
    ...
    if (gcpExtractorExists) {
      ...
      // Check for Firebase service account support (use proper URL/hostname validation for gserviceaccount.com references if URLs are present)
      const hasServiceAccountHost = (() => {
        // Attempt to extract URLs from code and validate hostnames
        const urlRegex = /(https?:\/\/[^\"'\s]+)/g;
        const matches = extractorCode.match(urlRegex) || [];
        return matches.some(u => {
          try {
            if (!validator.isURL(u, { require_protocol: true })) return false;
            const host = new URL(u).hostname;
            return host === 'iam.gserviceaccount.com' || host.endsWith('.iam.gserviceaccount.com') ||
                   host.endsWith('.gserviceaccount.com');
          } catch { return false; }
        });
      })();
      const hasFirebaseSupport = extractorCode.includes('firebase-adminsdk') || hasServiceAccountHost;
...
if (require.main === module) {
  const validatorRunner = new OptimizationValidator();
  validatorRunner.validate().then(() => {
    console.log('\n✅ Validation complete!');
  }).catch(error => {
