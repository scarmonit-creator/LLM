  ...
  "node_modules/whatwg-url/lib/url-state-machine.js"(exports2, module2) {
    ...
    function utf8PercentDecode(str) {
      ...
      for (let i2 = 0; i2 < input2.length; ++i2) {
        if (input2[i2] !== 37) {
          output2.push(input2[i2]);
        } else if (input2[i2] === 37 && isASCIIHex(input2[i2 + 1]) && isASCIIHex(input2[i2 + 2])) {
          output2.push(parseInt(input2.slice(i2 + 1, i2 + 3).toString(), 16));
          i2 += 2;
        } else {
          output2.push(input2[i2]);
        }
    ...
    function parseIPv6(input2) {
      ...
      while (pointer < input2.length) {
        ...
        while (length < 4 && isASCIIHex(input2[pointer])) {
          value = value * 16 + parseInt(at2(input2, pointer), 16);
          ++pointer;
          ++length;
        }
        if (input2[pointer] === 46) {
          ...
          while (input2[pointer] !== void 0) {
            ...
            while (isASCIIDigit(input2[pointer])) {
              const number = parseInt(at2(input2, pointer));
              if (ipv4Piece === null) {
                ipv4Piece = number;
              } else if (ipv4Piece === 0) {
                return failure;
    ...
    function parseOpaqueHost(input2) {
      ...
      for (let i2 = 0; i2 < decoded.length; ++i2) {
        output2 += percentEncodeChar(decoded[i2], isC0ControlPercentEncode);
      }
      return output2;
    }
    ...
    module2.exports.basicURLParse = function(input2, options) {
      if (options === void 0) {
        options = {};
      }
    ...
    module2.exports.parseURL = function(input2, options) {
      if (options === void 0) {
        options = {};
      }
      return module2.exports.basicURLParse(input2, { baseURL: options.baseURL, encodingOverride: options.encodingOverride });
    };
  }
});
  ...
  "node_modules/whatwg-url/lib/URL-impl.js"(exports2) {
    ...
    exports2.implementation = class URLImpl {
      ...
      set search(v2) {
        ...
        const input2 = v2[0] === "?" ? v2.substring(1) : v2;
        url.query = "";
  ...
  "node_modules/node-fetch/lib/index.js"(exports2, module2) {
    ...
    var Request3 = class _Request {
      constructor(input2) {
        ...
        const headers = new Headers3(init.headers || input2.headers || {});
  ...
  "node_modules/formdata-node/lib/esm/fileFromPath.js"() {
    ...
    FileFromPath = class _FileFromPath {
      constructor(input2) {
        ...
        __classPrivateFieldSet4(this, _FileFromPath_path, input2.path, "f");
        __classPrivateFieldSet4(this, _FileFromPath_start, input2.start || 0, "f");
        this.name = (0, import_path.basename)(__classPrivateFieldGet5(this, _FileFromPath_path, "f"));
        this.size = input2.size;
        this.lastModified = input2.lastModified;
      }
      slice(start, end) {
...
var BaseProvider = class {
  ...
  async setModel(_model) {
    throw new Error(`${this.label} does not support model switching.`);
  }
  async setSystemPrompt(_prompt) {
    throw new Error(`${this.label} does not support system prompts.`);
  }
  clearHistory() {
    this.history = [];
  }
...
var ClaudeProvider = class extends BaseProvider {
  constructor() {
    ...
    this.model = process.env.CLAUDE_MODEL?.trim() || process.env.MODEL?.trim() || "claude-3-5-sonnet-20241022";
    this.temperature = parseNumber(
      process.env.CLAUDE_TEMPERATURE ?? process.env.TEMPERATURE,
      1
    );
    this.maxTokens = parseNumber(
      process.env.CLAUDE_MAX_TOKENS ?? process.env.MAX_TOKENS,
      4096
    );
    this.systemPrompt = process.env.CLAUDE_SYSTEM_PROMPT?.trim() || "";
    this.client = this.apiKey ? new sdk_default({ apiKey: this.apiKey }) : null;
  }
  async isAvailable() {
  ...
  async setModel(model) {
    if (!model) {
      throw new Error("Provide a model identifier, e.g. `:model claude-3-haiku-20241022`.");
    }
  ...
  async send(message) {
    ...
    const reply = text || "[Claude returned an empty response]";
...
var OllamaProvider = class extends BaseProvider {
  constructor() {
    super("ollama", "Ollama");
    this.baseUrl = process.env.OLLAMA_API_BASE?.trim() || "http://localhost:11434";
    this.apiKey = process.env.OLLAMA_API_KEY?.trim();
    this.model = process.env.OLLAMA_MODEL?.trim() || "llama3.2";
    this.temperature = parseNumber(process.env.OLLAMA_TEMPERATURE, 0.7);
    this.maxTokens = parseNumber(process.env.OLLAMA_MAX_TOKENS, 512);
  ...
  async isAvailable() {
    try {
      ...
      if (Array.isArray(data.models)) {
        this.availableModels = data.models.map((model) => model.name).sort();
      }
      return true;
    } catch (error) {
      if (process.env.DEBUG_CHAT_LAUNCHER === "1") {
        console.error("[debug] Ollama availability check failed:", error);
      }
      return false;
    }
  }
  ...
  async listModels() {
    if (this.availableModels.length === 0) {
      return ["gemma2:2b", "llama3.2", "phi4"];
    }
    return this.availableModels;
  }
  async setModel(model) {
    if (!model) {
      throw new Error("Provide a model identifier, e.g. `:model llama3.2`.");
    }
    this.model = model.trim();
    return this.model;
  }
  ...
  async send(message) {
    ...
    const data = await response.json();
    const text = data?.message?.content || data?.message?.[0]?.content || "";
    const reply = text || "[Ollama returned an empty response]";
    this.history.push({ role: "assistant", content: reply });
    return reply;
  }
};
var EchoProvider = class extends BaseProvider {
  constructor() {
    super("echo", "Local Echo");
  }
  async isAvailable() {
    return true;
  }
...
function printBanner(providers) {
  console.log("============================================================");
  console.log("  Multi-Model Chat Launcher");
  console.log("  Type messages to chat. Commands are prefixed with a colon.");
  console.log("============================================================\n");
  if (providers.length === 0) {
    console.log("No providers detected. Configure ANTHROPIC_API_KEY or Ollama.");
    return;
  }
  console.log("Available providers:");
  providers.forEach((provider, idx) => {
    console.log(`  ${idx + 1}. ${provider.label}`);
  });
  console.log();
  console.log("Commands:");
  console.log("  :help                Show this help");
  console.log("  :providers          List detected providers");
  console.log("  :use <id|number>    Switch provider");
  console.log("  :model [name]       Show or update the active model");
  console.log("  :system [prompt]    Set system prompt (Claude only)");
  console.log("  :history            Print conversation history");
  console.log("  :clear              Clear current history");
  console.log("  :status             Show configuration snapshot");
  console.log("  :quit / :exit       Leave the chat");
  console.log();
}
async function main() {
  const providers = await detectProviders();
  if (providers.length === 0) {
    console.error("No chat providers available. Exiting.");
    process.exit(1);
  }
  let activeProvider = providers[0];
  printBanner(providers);
  console.log(`Active provider: ${activeProvider.promptLabel()}
`);
  const rl = (0, import_promises.createInterface)({ input: import_node_process.stdin, output: import_node_process.stdout });
  const getProviderLabel = () => `[${activeProvider.promptLabel()}]`;
  const renderHistory = () => {
    if (activeProvider.history.length === 0) {
      console.log("No history for this provider yet.");
      return;
    }
    console.log("--- Conversation History ---");
    activeProvider.history.forEach((entry, idx) => {
      console.log(`${idx + 1}. ${entry.role}: ${entry.content}`);
    });
    console.log("-----------------------------");
  };
  const showStatus = async () => {
    console.log("--- Status ---");
    console.log(`Provider: ${activeProvider.label}`);
    console.log(`Prompt label: ${activeProvider.promptLabel()}`);
    if (activeProvider instanceof ClaudeProvider) {
      console.log(`System prompt: ${activeProvider.systemPrompt || "[none]"}`);
    }
    if (activeProvider instanceof OllamaProvider) {
      console.log(`Base URL: ${activeProvider.baseUrl}`);
      console.log(`API Mode: ${activeProvider.apiKey ? "Cloud (API key)" : "Local server"}`);
    }
    console.log(`History length: ${activeProvider.history.length}`);
    console.log("---------------");
  };
  const handleModelCommand = async (rest) => {
    if (!activeProvider.supportsModelSwitch()) {
      console.log(`${activeProvider.label} does not support model changes.`);
      return;
    }
    if (!rest.trim()) {
      console.log(`Current model: ${activeProvider.promptLabel()}`);
      if (typeof activeProvider.listModels === "function") {
    ...
    const nextModel = await activeProvider.setModel(rest.trim());
    activeProvider.clearHistory();
    console.log(`Switched model to ${nextModel}. Conversation history cleared.`);
  };
  const handleSystemCommand = async (rest) => {
    if (!(activeProvider instanceof ClaudeProvider)) {
      console.log("System prompts are only available for Claude.");
      return;
    }
    const nextPrompt = await activeProvider.setSystemPrompt(rest);
    activeProvider.clearHistory();
    if (nextPrompt) {
      console.log("Updated system prompt and cleared conversation history.");
    } else {
      console.log("Cleared system prompt and conversation history.");
    }
  };
  const handleUseCommand = (rest) => {
    if (!rest) {
      console.log("Usage: :use <provider id|number>");
      return;
    }
    ...
    if (Number.isFinite(numeric) && numeric >= 1 && numeric <= providers.length) {
      next = providers[numeric - 1];
    } else {
      next = providers.find((provider) => provider.id.toLowerCase() === trimmed);
    }
    if (!next) {
      console.log(`Unknown provider "${rest}". Use :providers to see options.`);
      return;
    }
    activeProvider = next;
    console.log(`Switched to ${activeProvider.promptLabel()}.`);
  };
  const commandHandlers = /* @__PURE__ */ new Map([
    ["help", async () => printBanner(providers)],
    ["providers", async () => {
      providers.forEach((provider, idx) => {
        const isActive = provider === activeProvider ? "*" : " ";
        console.log(`${isActive} ${idx + 1}. ${provider.label} (${provider.id})`);
      });
    }],
    ["use", async (rest) => handleUseCommand(rest)],
    ["model", async (rest) => handleModelCommand(rest)],
    ["system", async (rest) => handleSystemCommand(rest)],
    ["history", async () => renderHistory()],
    ["clear", async () => {
      activeProvider.clearHistory();
  ...
  while (true) {
    const line = await rl.question(`${getProviderLabel()} `);
    const trimmed = line.trim();
    if (!trimmed) {
      continue;
    }
    if (trimmed.startsWith(":")) {
      const [cmd, ...restParts] = trimmed.slice(1).split(" ");
      const handler = commandHandlers.get(cmd.toLowerCase());
      if (!handler) {
        console.log(`Unknown command :${cmd}. Type :help for options.`);
        continue;
      }
      await handler(restParts.join(" "));
      continue;
    }
    try {
      const reply = await activeProvider.send(trimmed);
      console.log(`
${activeProvider.label}: ${reply}
`);
    } catch (error) {
      console.error(`Error from ${activeProvider.label}:`, error.message);
    }
  }
}
main().catch((error) => {
