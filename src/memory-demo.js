...
async function basicMemoryDemo() {
  ...
  // Add some memories
  await memory.addMemory('User prefers technical documentation with code examples', {
    importance: 0.8,
    type: MemoryType.SEMANTIC,
  });

  await memory.addMemory('User is working on an LLM project in Node.js', {
    importance: 0.9,
    type: MemoryType.SEMANTIC,
  });

  await memory.addMemory('Current conversation is about implementing working memory', {
    importance: 0.7,
...
async function memoryEnhancedLLMDemo() {
  ...
  await memoryLLM.rememberImportant('User is interested in vector databases for LLM applications', {
...
async function conversationContextDemo() {
  ...
  const conversation = [
    ...
    {
      role: 'assistant',
      content: "Sure! Here's a simple example of a neural network for image classification...",
    },
  ...
  const context = await memory.buildAugmentedContext('How do I train it?', { limit: 3 });
...
async function multiTypeMemoryDemo() {
  ...
  await memory.addMemory('User asked about machine learning on January 15, 2025', {
    type: MemoryType.EPISODIC,
    importance: 0.7,
  });
  ...
  await memory.addMemory('Currently discussing neural network architectures', {
    type: MemoryType.SHORT_TERM,
    importance: 0.6,
  });

  await memory.addMemory('Deep learning uses multiple layers of neural networks', {
